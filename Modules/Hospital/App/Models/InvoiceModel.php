...
class InvoiceModel extends Model
{
        ...
        $entity = self::where(function ($query) use ($id) {
            ...
            ->select([
             ...
             //   'hms_invoice.release_date as release_date',
                ...
                'admission_patient.diseases as diseases',
                'admission_patient.diseases_profile as diseases_profile',
                'admission_patient.clinical_findings  as clinical_findings',
    ...
    public static function getMedicineIssueDetails($params, $domain)
    {
        ...
        $endDate = $rawEnd ? Carbon::parse($rawEnd)->endOfDay() : ($rawStart ? Carbon::parse($rawStart)->endOfDay() : null);

        $warehouseId = $params['warehouse_id'] ?? null;

        $query = self::whereHas('prescriptions', function ($q) use ($warehouseId, $startDate, $endDate) {
            ...
            ->with([
                'customer',
                'room',
                'prescriptions' => function ($q) use ($warehouseId, $startDate, $endDate) {
                    $q->whereBetween('created_at', [$startDate, $endDate]);
        ...
        if ($perPage > 0) {
            $invoices = $query->skip($skip)->take($perPage)->get();
        } else {
            $invoices = $query->get();
        }
        ...
        foreach ($invoices as $invoice) {
            $row = [
                'patient_id' => $invoice->invoice,
                'room_no' => $invoice->room->display_name ?? null,
                'customer_name' => $invoice->customer->name ?? null,
            ];

            // Group prescriptions by stock_id and sum quantity
            $medicineSums = $invoice->prescriptions
                ->groupBy('stock_id')
                ->mapWithKeys(function ($items, $stockId) {
                    $stockName = $items->first()->stock->name ?? 'Unknown';
                    $totalQty = $items->sum('quantity');
                    return [$stockName => $totalQty];
                });

            // Add medicines as a single object
            $row['medicines'] = $medicineSums;

...
class PatientPrescriptionMedicineModel extends Model
{
                ...
                ->map(function ($medicine) use ($prescription,$date) {
                    ...
                    if ($medicineDetails) {
                        ...
                        return [
                            ...
                            'admin_status' => $medicineStock->admin_status,
                            'is_stock' => true,
    ...
    public static function insertTemplateMedicine($prescription,$medicines)
    {
        ...
        try {
            if (!empty($medicines)) {
                $prescription->prescription_medicine()->forceDelete();
                if($prescription->prescription_medicine()->count() === 0){
    ...
    public static function insertOpdTemplateMedicine($prescription,$medicines)
    {
                        ...
                        ->map(function ($medicine) use ($prescription,$date) {
                            ...
                            if ($medicineDetails) {
                                ...
                                return [
                                    ...
                                    'by_meal_bn' => $by_meal_bn,
                                    'continue_mode' => $continue_mode,
                                    'quantity' => $medicineStock->opd_quantity ?? 0,
                                    'opd_quantity' => $medicineStock->opd_quantity ?? 0,
                                    'ipd_status' => $medicineStock->ipd_status,
                                    'opd_status' => $medicineStock->opd_status,
                                    'admin_status' => $medicineStock->admin_status,
                                    'is_stock' => true,
    ...
    public static function ipdStockMedicine($prescription,$medicine)
    {
        $date = now();
        $medicineDetails = MedicineDetailsModel::find($medicine['medicine_id']);
        if (!$medicineDetails) {
    ...
    public static function ipdGenericMedicine($prescription,$medicine)
    {
        $date = now();
        $dose_details = '';
        $dose_details_bn = '';
    ...
    public static function ipdNewGenericMedicine($prescription,$medicine)
    {
        $date = now();
        $dose_details = '';
        $dose_details_bn = '';
        ...
        $genericDosage = $medicine['medicine_dosage_id'] ?? null;
        MedicineGenericModel::insertNewGeneric($medicine['medicine_name'],$medicine['generic'],$genericDosage);
